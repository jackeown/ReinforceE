...
def buildSmallerTables(hists, subFunc=lambda x: x):
	...
	for col in ["Run", "Processed", "vs Auto"]:	
	...
	for col in ["Run", "Generated", "vs Auto"]:
	...
	for col in ["Run", "Proof Clauses", "vs Auto"]:
...
if __name__ == "__main__":
	...
	path = f"figures/dists/{dataset}/generated.png"
	...
	path = f"figures/dists/{dataset}/generated_scaled.png"
	os.makedirs(os.path.split(path)[0], exist_ok=True)
	...
	path = f"figures/dists/{dataset}/processed.png"
	os.makedirs(os.path.split(path)[0], exist_ok=True)
	processedVis(hists, path, normalized=True, subFunc=subFunc2, dataset=dataset, xrange=[0,30_000])

	path = f"figures/dists/{dataset}/processed_scaled.png"
	os.makedirs(os.path.split(path)[0], exist_ok=True)
